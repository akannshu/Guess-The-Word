...
//This functions generates a random word
string random_word_generator_function()
{
  //srand functions sets the statring point to get a random value by a random function
  //It is necessary otherwise the random function will always print same values when recompiled
  srand(time(0));
  ...
  int selected_index;
  selected_index = 1 + (rand()%900); //A small trick is used to make random function in range of vector
...
// This function returns the updated state of the word and the attempts_remaining
pair<vector<char>,int> input_character_in_word(string selected_word,char input_char,vector<char> current_word_state, int attempts_remaining)
{
  ...
  //This loop checks whether the input of user in present in the word or not. If its present flag is set to 1 else 0
  //This uses another function which is called when the input character of the user matches with the character in the word
  for(int i=0; i<selected_word.length(); i++)
